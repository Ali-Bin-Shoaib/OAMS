import { Button, Card, Group, Loader, NumberInput, PasswordInput, Radio, Select, TextInput } from '@mantine/core';
...
export default function UserForm({ bigUser, userType, close }: Props): JSX.Element {
	...
	const {
		...
		setValue,
		formState: { errors },
	} = useForm<_BigUser>({ defaultValues: { ...bigUser, type: userType } });

	const onSubmit = async (data: _BigUser) => {
			...
			<form onSubmit={handleSubmit(onSubmit)}>
				<Card withBorder p={'xl'}>
					<Controller
						...
						render={({ field }) => {
							return (
								<TextInput {...field} label='name' error={errors.name && errors.name.message} placeholder='name' withAsterisk />
							);
						}}
					/>
					<Controller
