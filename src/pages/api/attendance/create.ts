...
import { Attendance, Orphan, Prisma, User, UserType } from '@prisma/client';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	try {
		if (req.method === REQUEST_METHODS.POST) {
					...
					OrphanAttendance: {
						createMany: {
							data: OrphanAttendance.map((x) => {
								const { id, attendanceId, userId, Orphan, ...rest } = x;
								return rest;
							}),
						},
					},
			...
			const newAttendance = await prisma.attendance.create(createAttendance);
		...
		return res.end(res.status(STATUS_CODE.METHOD_NOT_ALLOWED).json({ msg: 'Create New Attendance' }));
	} catch (error) {
